(function () {
  ...
  function ensureStyle() {
    ...
    style.textContent = [
      ...
      '@media (max-width:560px){.ar-serving-helper{grid-template-columns:1fr;margin:10px 0;padding:13px;max-width:100%;box-sizing:border-box}.ar-serving-helper strong{width:100%;min-height:40px;height:auto}.ar-serving-helper p{font-size:.95rem}}'
    ].join('');
  ...
  function enhance() {
    ...
    Array.prototype.slice.call(document.querySelectorAll('.ar-coach-result')).forEach(function (card) {
      ...
      existing.innerHTML = '<strong>' + esc(data.title) + '</strong><p>' + esc(data.body) + '</p>';
    });
  }
