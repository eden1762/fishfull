(function () {
  ...
  var applying = false;
  var timer = 0;
  var navVersion = '20260830-shared-header-v1';
  ...
  function installNavStyle() {
    ...
    style.textContent = [
      ...
      '.fishfull-shared-header .nav-actions,.fishfull-shared-header .actions{display:inline-flex!important;align-items:center!important;gap:7px!important;flex:0 0 auto!important}',
      '.fishfull-shared-header .circle-link,.fishfull-shared-header .icon-btn{width:42px!important;min-width:42px!important;height:42px!important;display:inline-grid!important;place-items:center!important;padding:0!important;border:1px solid rgba(8,91,108,.14)!important;border-radius:999px!important;background:#fff!important;color:#07566b!important;font-size:13px!important;font-weight:950!important;cursor:pointer!important;box-shadow:none!important;text-decoration:none!important}',
      '.fishfull-shared-header .circle-link:hover,.fishfull-shared-header .circle-link:focus-visible,.fishfull-shared-header .icon-btn:hover,.fishfull-shared-header .icon-btn:focus-visible{outline:none!important;background:#ffb23f!important;color:#291700!important}',
      '@media(max-width:1100px){',
      '.fishfull-shared-header{align-items:flex-start!important;flex-wrap:wrap!important;border-radius:22px!important}',
      '.fishfull-shared-header [data-fishfull-nav-signature]{display:flex!important;order:50!important;flex:1 0 100%!important;width:100%!important;max-width:100%!important;overflow-x:auto!important;overflow-y:hidden!important;flex-wrap:nowrap!important;justify-content:flex-start!important;gap:6px!important;padding:6px 0 2px!important;-webkit-overflow-scrolling:touch;scrollbar-width:none!important}',
      '.fishfull-shared-header [data-fishfull-nav-signature]::-webkit-scrollbar{display:none!important}',
      '.fishfull-shared-header [data-fishfull-nav-signature] a{flex:0 0 auto!important}',
      '}',
      '@media(max-width:620px){.fishfull-shared-header{top:5px!important;padding:9px!important;border-radius:18px!important}.fishfull-shared-header .circle-link,.fishfull-shared-header .icon-btn{width:38px!important;min-width:38px!important;height:38px!important}.fishfull-shared-header [data-fishfull-nav-signature] a{min-height:38px!important;padding:8px 10px!important;font-size:13px!important}}'
    ].join('\n');
  ...
  function makeInstagramLink(className) {
    var link = document.createElement('a');
    link.className = className || 'circle-link';
  ...
  function normalizeActions(header) {
    if (!header) return;
    var actions = header.querySelector('.nav-actions, .actions');
    if (!actions) {
    ...
    if (!ig) {
      var homeOnly = actions.querySelector('a[href="/"]');
      if (homeOnly && (homeOnly.textContent || '').trim() === '⌂') {
        ig = homeOnly;
        ...
        ig = makeInstagramLink('circle-link');
        actions.insertBefore(ig, actions.firstChild);
      }
    }
    ig.classList.add('circle-link');

    var langButton = actions.querySelector('#lang-toggle, .language-toggle');
    if (!langButton) {
    ...
    if (!langButton.querySelector('[data-lang-icon]')) {
      var text = (langButton.textContent || '').trim();
      langButton.textContent = '';
      var icon = document.createElement('span');
      icon.setAttribute('data-lang-icon', '');
      icon.textContent = text || (language() === 'en' ? '中文' : 'EN');
  ...
  function apply() {
    ...
    try {
      ...
      Array.prototype.forEach.call(document.querySelectorAll('header.fishfull-shared-header'), normalizeActions);
    } finally {
      applying = false;
    }
  }
